...
/*==========================Algorithm description, pthread===========================
In this progra, the loop scheduling use dynamic scheduling. 

In gauss function, initiate the mutex and creates the thread and call do_gauss function

Because the task is only respoinsible for one zero element in the lower diagonal of A, thus I 
add the mutex part inside the second loop. And the global and local row is responsibe for 
the dynamic scheduling part.

To ensure each row finishes its eleminating zero part, I add a barrier at the end of 
the inside loop. 

=================================================================================== */
 // create global norm lock
#include <pthread.h>
...
void gauss() {
  ...
  printf("Computing Paralleling.\n");

  pthread_t threads[procs];
  pthread_mutex_init(&global_row_lock, NULL);
  pthread_barrier_init(&barrier, NULL, procs);

  //create threads, perform parallel computing
  for(i = 0; i < procs; i++){
    //i is the argument
    pthread_create(&threads[i], NULL, &do_guass, (void*)i);
  }
...
void *do_guass(void *threadid){
  int norm, row, col;  /* Normalization row, * element row and col */
